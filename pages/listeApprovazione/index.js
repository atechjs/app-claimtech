...
import useListaApprovazioneAll from "../../components/fetching/useListaApprovazioneAll";

export default function Page() {
  const [state, actionSalvataggio] = useTableStateSaver();
  const { data: dataList, error, isLoading } = useListaApprovazioneAll();
  const router = useRouter();
  function openRecord(id) {
    actionSalvataggio.mantieni();
    router.push("/listeApprovazione/" + id);
  }

  function clickButtonNuovo() {
    actionSalvataggio.mantieni();
    router.push("/listeApprovazione/nuovo");
  }
  ...
  const tableOptions = {
    ...
    filterType: "dropdown",
    downloadOptions: {
      filename: "liste_di_approvazione.csv",
      separator: ",",
      filterOptions: {
  ...
  return (
    <Box p={2}>
      <MUIDataTable
        title={"Liste di approvazione registrate"}
        columns={columns}
