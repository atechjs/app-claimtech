...
import useAllStabilimenti from "../../components/fetching/useAllStabilimenti";

export default function Page() {
  const [state, actionSalvataggio] = useTableStateSaver();
  const { stabilimentiList } = useAllStabilimenti();
  const router = useRouter();
  function openRecord(id) {
    actionSalvataggio.mantieni();
    router.push("/stabilimenti/" + id);
  }

  function clickButtonNuovo() {
    actionSalvataggio.mantieni();
    router.push("/stabilimenti/nuovo");
  }
  ...
  return (
    <Box p={2}>
      <MUIDataTable
        title={"Stabilimenti registrati"}
        columns={columns}
        options={tableOptions}
        data={stabilimentiList}
      />
    </Box>
  );
}
