...
import useAllGruppiUtente from "../../components/fetching/useAllGruppiUtente";

export default function Page() {
  const [state, actionSalvataggio] = useTableStateSaver();
  const { gruppoUtenteList, error, isLoading } = useAllGruppiUtente();
  const router = useRouter();

  function openRecord(id) {
    actionSalvataggio.mantieni();
    router.push("/gruppiUtente/" + id);
  }

  function clickButtonNuovo() {
    actionSalvataggio.mantieni();
    router.push("/gruppiUtente/nuovo");
  }
  ...
  return (
    <Box p={2}>
      <MUIDataTable
        title={"Gruppi utenti registrati"}
        columns={columns}
        options={tableOptions}
        data={gruppoUtenteList}
      />
    </Box>
  );
}
