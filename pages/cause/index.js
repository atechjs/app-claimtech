...
import useCauseAll from "../../components/fetching/useCauseAll";
...
export default function Page() {
  ...
  const { causaList, error, isLoading } = useCauseAll();
  const router = useRouter();
  function openRecord(id) {
    actionSalvataggio.mantieni();
    router.push("/cause/" + id);
  }

  function clickButtonNuovo() {
    actionSalvataggio.mantieni();
    router.push("/cause/nuovo");
  }
  ...
  return (
    <Box p={2}>
      <MUIDataTable
        title={"Cause reclamo registrate"}
        columns={columns}
